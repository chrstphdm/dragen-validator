...
include { CHECK_LEGACY        } from '../modules/check_legacy'
...
process DELIVERABLE_STATUS_BEFORE_COPY{
    ...
    script:
    """
        if [[ \$CHECK_BASICS_STATUS == "ok" && \$CHECK_LEGACY_STATUS == "ok" && \$CHECK_METRICS_STATUS == "ok" ]]; then
...
process MERGE_TAP_FILES {
    ...
    script:
    """
        if [[ \$CHECK_BASICS_STATUS == "ok" && \$CHECK_LEGACY_STATUS == "ok" && \$CHECK_METRICS_STATUS == "ok" && \$CHECK_COPY_AND_FLAG_STATUS == "ok" ]]; then
            DELIVERABLE_STATUS="ok"
        else
            DELIVERABLE_STATUS="nok"
        fi
        echo "TAP version 14" > ${meta.dropbox_uuid}.${meta.data_type}.\${DELIVERABLE_STATUS}.tap
        cat ${check_basics_tapfile} ${check_legacy_tapfile} ${check_metrics_tapfile} ${check_copy_and_flag_tapfile} >> ${meta.dropbox_uuid}.${meta.data_type}.\${DELIVERABLE_STATUS}.tap
        echo "1..4" >> ${meta.dropbox_uuid}.${meta.data_type}.\${DELIVERABLE_STATUS}.tap
    """
}

workflow LEGACY_DATA {
    take:
        delivery_data
    main:
        // launching the first checks
        CHECK_BASICS(delivery_data,0)
        CHECK_LEGACY(delivery_data)
        CHECK_METRICS(delivery_data,1)
        // check for each data if it is deliverable or not
        DELIVERABLE_STATUS_BEFORE_COPY(CHECK_BASICS.out.join(CHECK_LEGACY.out.join(CHECK_METRICS.out)))
        ...
        MERGE_TAP_FILES(CHECK_BASICS.out.join(CHECK_LEGACY.out.join(CHECK_METRICS.out.join(CHECK_COPY_AND_FLAG.out.tap_reports))))
